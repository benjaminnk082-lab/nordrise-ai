...
> - Obsidian-sync ordering eller race conditions
...
4. **`config.ts.NORDRISE_REPO_URL` defaults to a DEAD GitHub account.** The default is `https://github.com/BennyK-tech/Nordrise-AI.git`, but Benjamin's `BennyK-Tech` GitHub account is no longer accessible — the active account is `benjaminnk082-lab`. The gateway's 30-min `git pull` tick is therefore either failing silently or pulling a stale snapshot. Fix: either change the default to `https://github.com/benjaminnk082-lab/nordrise-ai.git` or set `NORDRISE_REPO_URL` on Railway. **Not touched in this branch** — the default change is a runtime behavior change and outside the foundation scope; flag for the next session.
...
---
*Generated by Claude Opus 4.7 (1M context) during foundation work on 2026-05-05. Update freely as architecture evolves; keep §6 conservative.*
